...
IntegerMatrix gibbs_web_prior(Web<cluster_type,param_type,data_type>& W, NumericVector logA, NumericVector logB, int n_samples, int spacing) {
        ...
        for (int s=0; s<spacing; s++) {
            ...
            for (int rep=0; rep<2; rep++) {  // two restricted Gibbs sweeps (ATTN: maybe try doing more than 2?)
                for (int j=0; j<int(I.size()); j++) { // for each element in each of the two clusters
                    ...
                    double log_p1 = (c1->size()>0? logA[c1->size()] : logB[t]);
                    double log_p2 = (c2->size()>0? logA[c2->size()] : logB[t]);
