...
NumericVector unislicemNBD(NumericVector x1, double lx, IntegerVector Lm, NumericVector mu0, NumericVector hpriorpar, double w, int m, NumericVector lo, NumericVector up, IntegerVector samind) {
    ...
    for (int k0 = 0; k0<x1.size(); k0++) {
        if (samind[k0]==1){
            ...
            double gx0 = logpriorNBD(xx0, k0+1, xx2, Lm, mu0, hpriorpar) + lx;
            ...
            if (m==0)  // no limit on number of steps
            {
                //cout << "check-uni" << endl;
                while(L > lower)
                {   g = logpriorNBD(L, k0+1, xx2, Lm, mu0, hpriorpar) +lx;
                ...
                while(R < upper)
                {   g = logpriorNBD(R, k0+1, xx2, Lm, mu0, hpriorpar) + lx;
                    if (g <= logy) break;
                    R = R + w;
                }
            ...
            do
            {
                ...
                gx2 = logpriorNBD(x2, k0+1, xx2, Lm, mu0, hpriorpar) + lx;
...
// [[Rcpp::export]]
NumericVector unislicem1(NumericVector x1, int N, int Khat, double lx,
                        IntegerVector Nk, NumericVector hpriorpar, double w, int m, NumericVector lo,
                        NumericVector up, std::string Prior, IntegerVector samind) {
    NumericVector xx2 = x1;
